...
export const MIN_VIEW_SIZE_M = 0.5;
export const MAX_VIEW_SIZE_M = 1000;
...
export const viewToStageTransform = (view: ViewportView, viewportPx: { width: number; height: number }): StageTransform => {
  const scale = viewportPx.width / view.w;
  return {
    x: -view.x * scale,
    y: -view.y * scale,
    scale
...
export const withPanDelta = (
  ...
  return {
    ...view,
    x: view.x - deltaPx.x * metersPerPixel,
    y: view.y - deltaPx.y * metersPerPixel
  };
};

export const withZoomAtScreenPoint = (
  view: ViewportView,
  viewportPx: { width: number; height: number },
  screenPoint: Vec2,
  zoomFactor: number
...
export const getGridStepWithLod = (baseStepM: number, pixelsPerMeter: number, minSpacingPx = 8): number => {
  ...
  for (let i = startIndex; i < steps.length; i += 1) {
    if (steps[i] * pixelsPerMeter >= minSpacingPx) return steps[i];
  }
